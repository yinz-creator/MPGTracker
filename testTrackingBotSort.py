...
def cv2_demo(frame, detections):
    ...
    for i in range(detections.shape[0]):
        if detections[i, 4] >= CONFIDENCE_thres:
            ...
            #confidence_text = "{:.1f}".format(pt[4])
            #cv2.putText(frame, confidence_text, (int(pt[0]), int(pt[1])), FONT, 1, (0, 255, 0), 1)
...
def test(opt, split, modelPath, show_flag, results_name):
    ...
    if saveTxt:
        ...
        track_results_save_dir = os.path.join(opt.save_results_dir, 'trackingResults'+'botsort')
        if not os.path.exists(track_results_save_dir):
            os.mkdir(track_results_save_dir)

    num_iters = len(data_loader)
    bar = Bar('processing', max=num_iters)
    video1 = cv2.VideoWriter('botsort_test.avi',cv2.VideoWriter_fourcc(*'MJPG'),5,(1280,720))
    video2 = cv2.VideoWriter('/root/autodl-tmp/zjr/Moving-object-detection-DSFNet-main/test2.avi',cv2.VideoWriter_fourcc(*'MJPG'),5,(1280,720))
    ...
    for ind, (file_path, img_id, pre_processed_images) in enumerate(data_loader):
        ...
        imgOri = pre_processed_images['imgOri'].squeeze().numpy()
        ...
        online_targets,cc = mot_tracker.update(dets_track, imgOri)
        
        #print(track_bbs_ids)
        track_bbs_ids = []
        for t in online_targets:
            #print(t.tlwh)
            box = [t.tlwh[0], t.tlwh[1], t.tlwh[0] + t.tlwh[2], t.tlwh[1] + t.tlwh[3],t.track_id]
            track_bbs_ids.append(box)
        track_bbs_ids = np.array(track_bbs_ids).reshape(-1, 5)
        #print(track_bbs_ids.shape)

        if(show_flag):
            frame, det = cv2_demo(imgOri, track_bbs_ids)
            #读取图片
                
   	        # resize方法是cv2库提供的更改像素大小的方法
            # 将图片转换为1280*720像素大小
            img1 = cv2.resize(frame,(1280,720))
            cv2.imwrite('./botsort_test.jpg',frame)
            # 写入视频
            video1.write(img1)
